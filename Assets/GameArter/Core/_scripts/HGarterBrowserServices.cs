...
public class HGarterBrowserServices {
	...
	public void AdRequest(AdUnitData adData, Action<string> callback)
    {
        ...
        if (adData.adType == AdsConfiguration.AdType.Fullscreen)
        {
            ...
            if (callback != null) {
                _gb_adRequest(JsonUtility.ToJson(new SpData(6, adData.mutedGame, adData.cursorLockState, adData.visibleCursor))); 
            } else {
                Debug.LogError("Callback function must be defined for rewarded ads");
            }
        }
	}
