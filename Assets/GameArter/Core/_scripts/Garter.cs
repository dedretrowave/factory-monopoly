// Copyright 2013 - 2020, Awagon Entertainment s.r.o., All rights reserved._Rew
using System;
using System.Linq;
using UnityEngine;
...
public class Garter : MonoBehaviour {
    private readonly static string sdkVersion = "3.5";
    ...
    private string[] multiplNetwork = null; //gamearter photonId and version (null = is not required - own network solution / singleplayer)
    ...
    */
    ...
    private class IEvent
    {
        ...
        public bool le; // active leaderboard
    }
    ...
    AdvertisementState advertisementState = AdvertisementState.empty;
    ...
    /// <summary>
    /// SDK communication function. Do not use it.
    /// </summary>
    public void HRewardedAdIsReady(int available)
    {
        rewardedAdsEnabled = (available == 1);
    }

    /// <summary>
    /// Returns whether a rewarded ad is available to be displayed
    /// </summary>
    /// <returns><c>true</c>, if ad is available, <c>false</c> otherwise.</returns>
    public bool RewardedAdAvailability() {
        return rewardedAdsEnabled;
    }

    public bool RewardedAdsAvailability() {
        // [System.Obsolete("RewardedAdsAvailability is obsolete method. Switch at Garter.I.RewardedAdAvailability()")]
        return rewardedAdsEnabled;
    }
   ...
   /// <returns><c>true</c> if browser tab with game is active; otherwise, <c>false</c>.</returns>
    ...
    public decimal LocalCurrency(decimal change = 0M) {
        if (change != 0M) {
            ...
            prCheck.sum += change;
            UpdateUserBoard();
            if (autoSaveEnabled) StartCoroutine(SafePostRequest(false, false, null));
        }
        return (localCurrency - hkey.lc);
    }

    public decimal LocalCurrencyAbs(decimal absoluteValue) { //8 //3
        decimal curValue = LocalCurrency(0); //3 //8
        decimal diff = absoluteValue - curValue; // 8-3=5 // 3-8=-5
        return LocalCurrency(diff);
    }
    ...
    private void DefineNextBadge(byte reqType, float[] nbValues, string[] platformId) {
        ...
        {
            if (reqType == 1 || reqType == 3 || reqType == 6)
            { // init load - write badges
    ...
    private void AttachLeaderboard(byte[] leaderboardEvents)
    {
        if(leaderboardEvents != null)
        {
            for (int i = 0; i < events.Length; i++)
            {
                if (System.Array.IndexOf(leaderboardEvents, i) != -1) events[i].le = true;
            }
        } // else default value = false
    }

    private void SetCoinExperience(float[] fCoinsExp)
    {
        if (fCoinsExp != null)
        {
            int[] coinsExp = MultiplyByThousand<int>(fCoinsExp);
    ...
    private void DetectPlatform() {
        ...
        else if (platform == RuntimePlatform.LinuxPlayer)
        {
            ...
            gui.IlustrateBrowserBox("Error", "Unsupported or unknown platform " + platform.ToString() + ". Be sure you have active Android or WebGL project in Build settings. Contact support if you need a help");
    ...
    public void Initialize(object[] initializeData) {
        if (initializeData != null) { //Initialize complet
            ...
            if (platformServices == null) Debug.LogWarning("Unknown platform services. Contact support - vladimir@gamearter.com");
            ...
            if (System.DateTime.Now.Year < 0) { // actually never run but Unity doesn't know :)
                ...
                Debug.Log(new Dictionary<string, int>());
                /*Debug.Log(new WebPlayerInitData());
                Debug.Log(new MobilePlayerInitData());
                Debug.Log(new ReceivedDataFull());
                Debug.Log(new ReceivedDataLite());*/
            }
            ...
            if (activeSdk) {
                ...
                isMultiplayer = (bool)initializeData[6];
                ...
                if (!interpreter.Equals('B')) { //Full or Lite SDK
                    ...
                    // requires gamearter network solution
                    if ((bool)initializeData[10]) multiplNetwork = new string[2] { "", "" }; //multiplayer required
                    ...
                    if (targetPlatform == Platform.Web)
                    {
                        ...
                        { //run game - download user data -> gameplayer -> database
                            ...
                            { // || file // Editro Mode = false
                                ...
                                platformServices.InitGameSignature(sdkVersion, interpreter, (uint)initializeData[0], isMultiplayer, projectVersion);
                            }
    ...
    public void EditorMode(string init) { // skip/replace SdkInitCallback
        if (!interpreter.Equals('B')) {
            ...
            if (isMultiplayer && multiplNetwork != null) { // get network... - basic sdk
                asyncCallbacks = 2;
    ...
    public void SdkInitCallback(string data) { // init call from browser / editor init
        ...
        if (targetPlatform == Platform.Web)
        {
            ...
            { // async callback 1
                ...
                else if (isMultiplayer && multiplNetwork != null)
    ...
    /// <summary>
    /// Posts data to leaderboard
    /// </summary>
    /// <param name="eventName">Event srting id</param>
    /// <param name="absEventValue">Achieved value of an event</param>
    public void PostToLeaderboard(string eventName, decimal absEventValue)
    {
        if (Event(eventName) != absEventValue)
        {
            EventAbs(eventName, absEventValue);
            StartCoroutine(SafePostRequest(false, true, null));
        } else
        {
            Debug.LogWarning("Same user score is already in leaderborad");
        }
    }
    ...
    public decimal EventAbs(string eventName, decimal absEventValue) {
        ...
        if (index != -1) { // event Index
            decimal sValue = events[index].v - hkey.e;
            if (events[index].t == 0) // increasing
            {
                if (absEventValue > sValue) // ok
                {
                    return Event(eventName, (absEventValue - sValue));
                } else // fail
                {
                    if(absEventValue != sValue) Debug.LogWarning("Abs value in a clash with defined event trend");
                    return sValue;
                }
            } else if (events[index].t == 1)
            {
                if(absEventValue < sValue) // ok
                {
                    return Event(eventName, (absEventValue - sValue));
    ...
    float achievementProgress = 0f;
                    ...
                    if (eventData.t < 2) {
                        if (eventData.t == 0) { //event with increasing trend - kills, deaths...
                            ...
                            if ((eventData.nbv != -1 && (eventData.v - hkey.e) >= (decimal)eventData.nbv) || (eventData.le && GetCurrentTimestamp() - lastSave > 120)) impEvent = true; // received achievement / active leaderboard
    ...
    public void HadvertisementStateCb(string state)
    {
        switch (state)
        {
            case "empty": advertisementState = AdvertisementState.empty; break;
            case "pending": advertisementState = AdvertisementState.pending; break;
            case "loaded": advertisementState = AdvertisementState.loaded; break;
            case "running": advertisementState = AdvertisementState.running; break;
            case "completed": advertisementState = AdvertisementState.completed; break;
            case "failed": advertisementState = AdvertisementState.failed; break;
            case "ignored": advertisementState = AdvertisementState.ignored; break;
            default:
                Debug.LogWarning("Unknown ad state format returned - " + state);
                break;
        }

        if (debugMode) Log("info","advertisement state: " + advertisementState.ToString());

        string[] finalStates = { "completed", "failed", "ignored" };
        bool isCompleted = finalStates.Contains(state);
           
        // callback functons
        if (cachedCallbacksListeners.ContainsKey(_CachedListener._AdState))
        {
            ForwardCachedCb(_CachedListener._AdState, isCompleted, advertisementState.ToString());
        }
        else if(advertisementState == AdvertisementState.loaded)
        {
            SdkWindowOpened("ad");
        } else if (advertisementState == AdvertisementState.completed || advertisementState == AdvertisementState.failed)
        {
            SdkWindowClosed("ad");
        }
        if (isCompleted) advertisementState = AdvertisementState.empty;
    }
...
#if UNITY_WEBGL
        if (advertisementState == AdvertisementState.empty)
        {
            platformServices.AdRequest(unitData, callback);
            if (unitData.adType == AdsConfiguration.AdType.Rewarded) rewardedAdsEnabled = false; // disabled into next rewarded ad initialization
        }
        else
        {
            Debug.LogError("Ad request rejected. Previous ad has not been completed yet");
        }
#else
    ...
    {
        // check, whether requested ad unit is defined
        ...
        if (!unitData.defined) Debug.LogWarning("Ad unit id was not defined. Define or request defined ad unit.");
        return unitData;
    }
	...
	internal class ServerObjGetInit : ServerObjGet
	{
		...
		public byte g; //gift
        public byte[] le;
		public ServerObjGetInit(string[] network, byte[] iconsVisibility, byte gift, byte[] leaderboardEvents, string data, string hash,byte[] alert, string[] badges, string[] badgesUrl, string[] badgesPids, float[] nextBadgeVal, string[] items, uint[] leaderboard):base(data,hash,alert,badges,badgesUrl,badgesPids,nextBadgeVal, items,leaderboard){
			this.n = network;
			this.i = iconsVisibility;
			this.g = gift;
            this.le = leaderboardEvents;
		}
	}
